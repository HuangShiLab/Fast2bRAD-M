#!/bin/bash
#SBATCH --job-name=holo_ecc_small
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=04:00:00
#SBATCH --output=holo_ecc_small_%j.log
set -euo pipefail

# Small-batch pilot: first 3 ECC samples
METADATA="/lustre1/g/aos_shihuang/holo2bRAD/caries_pilot/metadata.tsv"
ENZYME="BcgI"
OUTDIR="/lustre1/g/aos_shihuang/holo2bRAD/caries_pilot/smallbatch"
THREADS="${1:-8}"
...
awk -F'\t' 'NR>1 && NR<=4 && $2!="" {print $1"\t"$2}' "${METADATA}" > "${SAMPLE_LIST}"
