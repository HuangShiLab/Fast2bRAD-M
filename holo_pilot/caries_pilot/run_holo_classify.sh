#!/bin/bash
#SBATCH --job-name=holo_ecc
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=holo_ecc_%j.log
set -euo pipefail
...
# Build sample list from metadata: sample_id<TAB>r1 (single-end ECC data)
SAMPLE_LIST="${OUTDIR}/sample_list.tsv"
awk -F'\t' 'NR>1 && $2!="" {print $1"\t"$2}' "${METADATA}" > "${SAMPLE_LIST}"
...
echo "Done. Per-sample outputs are in ${OUTDIR}/holo/<sample_id>/"
