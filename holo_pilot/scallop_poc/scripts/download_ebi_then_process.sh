...
URL="ftp://ftp.sra.ebi.ac.uk/vol1/fastq/SRR202/008/SRR2027758/SRR2027758.fastq.gz"
PART="$WORKDIR/reads/fastq/SRR2027758.fastq.gz.partial"
FINAL="$WORKDIR/reads/fastq/SRR2027758.fastq.gz"

echo "$(date): Downloading FASTQ from EBI..."
wget -c -O "$PART" "$URL" > "$WORKDIR/results/wget_download.log" 2>&1
echo "$(date): Download finished."

mv "$PART" "$FINAL"

echo "$(date): Decompressing FASTQ..."
cd "$WORKDIR/reads/fastq"
pigz -dk "$FINAL"
ls -lh

echo "$(date): Splitting pooled FASTQ by sample prefix..."
python3 - << "PYEOF"
import gzip, re, os
...
echo "$(date): Selecting first 3 samples for proof-of-concept..."
head -4 "$WORKDIR/reads/by_sample/samples.tsv" > "$WORKDIR/reads/samples.tsv"

echo "$(date): Submitting f2brad-holo classify job..."
