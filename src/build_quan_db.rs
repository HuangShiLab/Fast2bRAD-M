...
pub fn run(args: BuildQuanDbArgs) -> Result<()> {
    ...
    let enzyme_records = load_enzyme_records(&intermediate_enzyme_file)?;
    tracing::info!("Loaded {} enzyme records", enzyme_records.len());


    for level in &levels {
        tracing::info!("\n========== Building {}-level database (Hash mode) ==========", level.as_str());
        build_database_for_level(&enzyme_records, enzyme, &args.output_dir, *level, &genome_records, remove_redundant)?;
    }
    tracing::info!("\nAll done!");
    Ok(())
}
...
fn build_database_for_level(
    ...
    let mut processed_gcfs: FxHashSet<&str> = FxHashSet::default();

    for rec in records {
        if let Some(taxonomy) = gcf_to_taxonomy.get(rec.gcf_id.as_str()) {
            processed_gcfs.insert(rec.gcf_id.as_str());
